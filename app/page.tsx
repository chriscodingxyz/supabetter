...
export default function Home() {
  ...
  return (
    <div className='bg-background relative min-h-[calc(100dvh-60px)] lg:h-[calc(100vh-60px)] lg:overflow-hidden'>
      ...
      {/* Main content grid - compact for single screen on desktop only */}
      <div className='container px-4 mx-auto h-full flex flex-col'>
        {/* Hero Section - more compact on desktop only */}
        <div className='flex-1 grid lg:grid-cols-[1fr_1.618fr] gap-8 lg:gap-12 items-center py-24 md:py-32 lg:py-12'>
          
          {/* Left: Primary content - responsive spacing */}
          <div className='relative order-1 lg:order-1'>
            <div className='space-y-8 lg:space-y-6'>
              ...
              <div className='space-y-4 lg:space-y-3'>
                <h1 className='flex flex-col text-4xl sm:text-5xl lg:text-5xl font-bold tracking-tight'>
                  <MetallicText>Skip the setup.</MetallicText>
                  <MetallicText theme='rainbow'>Start the build.</MetallicText>
                </h1>
                
                <p className='text-lg sm:text-xl lg:text-lg text-muted-foreground max-w-xl'>
              ...
              {/* CTA Buttons */}
              <div className='flex flex-row gap-4 pt-2 lg:pt-0'>
                <Link href='/build'>
          ...
          {/* Right: Visual element - responsive scaling */}
          <div className='relative order-2 lg:order-2 hidden md:block'>
            <div className='relative mx-auto md:max-w-none lg:scale-90 xl:scale-100'>
              {/* Interactive Terminal Setup */}
              <InteractiveTerminal />

              {/* Stats badges */}
              <div className='absolute -bottom-6 lg:-bottom-4 left-1/2 -translate-x-1/2 flex gap-3'>
        ...
        {/* Feature slider - using infinite slider with hover speed */}
        <div className='border-t border-border/50 pt-16 pb-24 lg:py-8 w-screen relative left-1/2 -translate-x-1/2'>
          <div className='relative'>
            {/* Mobile & Small Screens: 2 Rows */}
            <div className="hidden md:block space-y-4">
              ...
              >
                {[...featuresData.slice(0, 4), ...featuresData.slice(0, 4)].map((feature, index) => (
                  <div
                    key={index}
                    className='group relative p-6 lg:p-5 rounded-xl bg-card/50 backdrop-blur-sm border border-border/50 hover:border-border transition-all duration-300 min-w-[300px] lg:min-w-[280px] max-w-[340px] lg:max-w-[320px]'
                  >
            ...
            {/* Medium & Large Screens: 3 Rows */}
            <div className="md:hidden space-y-4">
              {/* Row 1 (md) - moving left - Features 1-3 */}
              <InfiniteSlider
                gap={24}
                speed={80}
                speedOnHover={0.1}
              ...
              {/* Row 2 (md) - moving right - Features 4-6 */}
              <InfiniteSlider
                gap={24}
                speed={80}
                speedOnHover={0.1}
              ...
              {/* Row 3 (md) - moving left - Features 7-8 */}
              <InfiniteSlider
                gap={24}
                speed={80}
                speedOnHover={0.1}
